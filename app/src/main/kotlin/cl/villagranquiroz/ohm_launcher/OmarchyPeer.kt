...
data class OmarchyPeer(
    ...
    companion object {
        fun fromJson(json: JSONObject): OmarchyPeer? {
            ...
            return OmarchyPeerUri.parse("omarchy://$authority:$port?id=$encodedId")
        }
    }
}

/** Strict parser for `omarchy://host:port?id=name` discovery links. */
object OmarchyPeerUri {
    fun parse(value: String): OmarchyPeer? {
        ...
        if (query.keys.any { it != "id" }) return null
        ...
        return OmarchyPeer(host, uri.port, id)
    }
